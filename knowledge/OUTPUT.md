...
- `src/core/fast_sam_3dbody.h` / `fast_sam_3dbody_capi.h` — the `MHRResult` / `FsbResult` struct
...
- `src/core/mhr_joint_table.h` — the **127** MHR LBS skeleton joint names + parents
...
The full body-model skeleton has **127** named joints (`src/core/mhr_joint_table.h`,
...
> names via the `NAME_MAP` table in `src/core/bvh_writer.cpp`; unmapped joints (toes,
