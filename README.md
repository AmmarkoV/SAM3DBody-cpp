...
└── src/                              All C/C++ sources live here
    ...
    │   ├── bvh_writer.h / .cpp       BVH motion-capture exporter (multi-person, name-mapped to MHR joints)
    │   ├── offline_passes.h / .cpp   Multi-pass offline orchestration (shared with src/multiview/)
    │   ├── mhr_joint_table.h         Generated by tools/build_joint_table.py — MHR joint names + parents
    │   └── main.cpp                  CLI executable (--bvh, --out CSV, live overlay window)
    ├── render/
    │   ├── fast_sam_3dbody_render.cpp    OpenGL mesh overlay renderer
    │   ├── offline_sam_3dbody_render.cpp Offline multi-pass BVH extractor
    ...
    ├── ros1/                         Standalone ROS1 catkin package (poseEstimation3D node, remote server)
    ├── ros2/                         Standalone ROS2 ament package (poseEstimation3D node, local engine)
    ├── GraphicsEngine/
    ...
    └── SharedMemoryVideoBuffers/     Zero-copy shm transport (cloned by tools/setup_gmr.sh)
```
...
python3 tools/verify_bvh_motion.py ./p_0.bvh /tmp/boom_mhr.csv
```
...
* The BVH I/O comes from a vendored, trimmed-down copy of
  ...
  in `src/GraphicsEngine/MotionCaptureLoader/` (plus `TrajectoryParser/InputParser_C`).
...
`src/render/offline_sam_3dbody_render.cpp` is **only** the per-pass
